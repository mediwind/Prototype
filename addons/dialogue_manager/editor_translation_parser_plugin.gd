...
func _parse_file(path: String) -> Array[PackedStringArray]:
	...
	var result_data: Array[PackedStringArray] = []
	...
	if DMSettings.get_setting(DMSettings.INCLUDE_CHARACTERS_IN_TRANSLATABLE_STRINGS_LIST, true):
		...
		for character_name: String in data.character_names:
			...
			# [source, context, plural]
			result_data.append(PackedStringArray([character_name.replace('"', '\"'), "dialogue", ""]))
	...
	for key: String in dialogue.keys():
		...
		if translation_key == line.text:
			result_data.append(PackedStringArray([line.text.replace('"', '\"'), "", ""]))
		else:
			result_data.append(PackedStringArray([line.text.replace('"', '\"'), line.translation_key.replace('"', '\"'), ""]))

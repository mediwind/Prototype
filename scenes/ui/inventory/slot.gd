...
func _drop_data(_at_position: Vector2, data: Variant) -> void:
	...
	var move_amount = data.amount if "amount" in data else source_slot_data_ref.amount
	...
	# 그 외에는 SlotData swap (자기 자신에게 드랍하는 경우는 제외)
		...
		# split 드래그(일부만 옮기는 경우)
		if "split" in data and data.split and move_amount < source_slot_data_ref.amount:
			# 타겟이 비어있으면 일부만 옮김
			if not target_slot_data_ref.item_data:
				target_slot_data_ref.item_data = source_slot_data_ref.item_data
				target_slot_data_ref.amount = move_amount
				source_slot_data_ref.amount -= move_amount
				if source_slot_data_ref.amount <= 0:
					source_slot_data_ref.item_data = null
					source_slot_data_ref.amount = 0

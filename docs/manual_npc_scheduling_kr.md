...
## 🧭 6. 개발 철학 및 미래 확장 (Design Philosophy)

### Q. NPC가 제시간에 도착 못 하면 어떻게 하죠? (Simulation vs Correction)
현재 시스템은 **"Real-time Simulation (Immediate Diversion)"** 방식을 따릅니다.
*   **작동 방식:** NPC는 목적지가 바뀌면, 현재 위치에서 즉시 발길을 돌려 새로운 목적지로 걸어갑니다.
*   **장점:** 플레이어가 NPC를 따라다닐 때, NPC가 실제로 이동하는 과정이 보여 생동감(Living World)을 줍니다.
*   **미래의 문제:** 맵이 거대해지면 NPC가 하루 종일 길바닥에만 있을 수 있습니다.
*   **해결책 (Catch-up / Teleport):**
    추후 맵이 넓어지거나 퀘스트 컷신 등으로 "반드시 그 자리에 있어야 하는" 상황이 오면, **Catch-up(안 볼 때 속도 3배)** 또는 **Teleport(안 볼 때 순간이동)** 기법을 도입하여 보정합니다. 지금은 가장 정직하고 기초적인 시뮬레이션 방식을 유지합니다.
